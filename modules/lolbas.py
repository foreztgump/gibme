...
def _lolbas_parse_info(client, exe_name):
    ...
    console.print(f"  Description: [spring_green3]{data['Description']}[/spring_green3]")
    ...
    for i, command in enumerate(data['Commands'], start=1):
        ...
        for key, value in command.items():
            if key in ['Command', 'Code', 'Path']:
                ...
                value = re.sub(r'"(.*?)"', r'[bold magenta]\1[/bold magenta]', str(value)) 
                console.print(f"  {key}: [spring_green3]{value}[/spring_green3]")

    for section in ['Full_Path', 'Code_Sample', 'Detection', 'Resources', 'Acknowledgement']:
        if section in data:
